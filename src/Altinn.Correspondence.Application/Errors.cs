...
public static class StatisticsErrors
{
    ...
    public static Error InvalidReportMonth = new Error(6004, "Year and Month must both be provided and valid when specifying a report period. Day is optional (1-31) for a single-day report and defaults to the preceding UTC day when omitted.", HttpStatusCode.BadRequest);
    ...
    public static Error ReportDayNotComplete = new Error(6006, "Reports cannot be generated for the current or future UTC day because the day is not finished yet.", HttpStatusCode.BadRequest);
}