...
export async function retrieveMaskinportenToken({ clientId, kid, pem, scope }) {
    const jwt = await buildMaskinportenJwt({ clientId, kid, pem, scope, tokenUrl: maskinportenUrl });
...
export async function getMaintenanceMaskinportenToken() {
    return await retrieveMaskinportenToken({
        ...
        scope: 'altinn:correspondence.maintenance',
    });
}