...
namespace Altinn.Correspondence.Tests.TestingHandler
{
    public class ExpireAttachmentHandlerTests
    {
        ...
        [Fact]
        public async Task Process_Skips_WhenExpireIdempotencyKeyExists()
        {
            var attachmentId = Guid.NewGuid();
            var partyUuid = Guid.NewGuid();
            ...
            _altinnRegisterServiceMock
                ...
                .ReturnsAsync(RegisterServiceMockExtensions.BuildOrganization(partyUuid, "991825827"));
            _idempotencyKeyRepositoryMock
                .Setup(r => r.GetByIdAsync(expireIdempotencyId, It.IsAny<CancellationToken>()))
                .ReturnsAsync(new IdempotencyKeyEntity { Id = expireIdempotencyId });

            var result = await _handler.Process(attachmentId, null, CancellationToken.None);

            Assert.True(result.IsT0);
            _attachmentStatusRepositoryMock.Verify(
                x => x.AddAttachmentStatus(It.IsAny<AttachmentStatusEntity>(), It.IsAny<CancellationToken>()),
                Times.Never);
        ...
        [Fact]
        public async Task Process_Skips_WhenUniqueViolationOnFlush()
        {
            var attachmentId = Guid.NewGuid();
            var partyUuid = Guid.NewGuid();
            var attachment = new AttachmentEntity
            ...
            var handler = new ExpireAttachmentHandler(
                _loggerMock.Object,
                _attachmentRepositoryMock.Object,
                _attachmentStatusRepositoryMock.Object,
                _idempotencyKeyRepositoryMock.Object,
            ...
            var result = await handler.Process(attachmentId, null, CancellationToken.None);
